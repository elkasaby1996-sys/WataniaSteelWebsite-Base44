...
// Weight per meter in kg for each diameter
const weightPerMeter = {
  8: 0.395, 10: 0.617, 12: 0.888, 14: 1.21, 16: 1.58,
  18: 2.00, 20: 2.47, 22: 2.98, 25: 3.85, 32: 6.31
};
...
export default function Order() {
  ...
  const expressFee = formData.is_express ? 500 : 0;

  const submitMutation = useMutation({
    mutationFn: async (data) => {
      let boqUrl = null;
      if (boqFile) {
        const result = await base44.integrations.Core.UploadFile({ file: boqFile });
        boqUrl = result.file_url;
      }
      ...
      return base44.entities.Order.create({
        ...
        items: items.map(item => ({
          ...item,
          total_weight: parseFloat(calculateWeight(item))
        })),
    ...
    <div className="min-h-screen pt-20 bg-gray-50">
      ...
      <section className="bg-gradient-to-br from-[#1A1A1A] to-[#2d2d2d] py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <span className="text-[#7B1F32] font-semibold tracking-wide uppercase text-sm">Order</span>
            <h1 className="text-4xl lg:text-5xl font-bold text-white mt-4 mb-4">
              Place Your Order
            </h1>
            <p className="text-xl text-gray-400 max-w-2xl">
              Choose your ordering method and submit your requirements directly.
            </p>
          </motion.div>
        </div>
      </section>
                  ...
                  <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-xl p-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-gray-600">Priority processing (+500 QAR)</div>
                      </div>
                    </div>
                    <Switch
